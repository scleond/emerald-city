...
def read(): return read_file(path())
def write_file(p,s):
...
def main(a):
    ...
    if a.command=='record':
        ...
        if a.kind=='verification':
            ...
            if a.value not in ('pushed','closed'):fail('Remote observation must be pushed or closed.')
            changed=add_unique(s,'observations',a.value)
    ...
    if a.command=='permission':
        ...
        s['permissionAttempts']+=1;s['handledPermissionIds'].append(a.permission_id);return save(s)
