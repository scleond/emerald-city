...
import { OPENCODE_GO_SOURCE_DATE, OPENCODE_GO_PRICING_VERSION } from "./opencode-go-pricing";
...
function ReadyContent({ view, historicalUsage, styles, telemetry, lifecycle, setLifecycle, selectedAgentId, selectAgent, loadMore, timeline, attention, onDismiss, attentionOpen, toggleAttention, historyRange, setHistoryRange }: { view: ObservatoryViewModel; historicalUsage?: Record<UsageRange, HistoricalUsageProjection>; styles: PanelStyles; telemetry?: TelemetryDiagnostic; lifecycle?: AgentLifecycle; setLifecycle: (value: AgentLifecycle | undefined) => void; selectedAgentId: string | null; selectAgent: (id: string) => void; loadMore: (id: string) => void; timeline: Record<string, { entries: { label: string; summary: string; at: string }[]; error?: string; hasOlder: boolean }>; attention: AttentionEntry[]; onDismiss: (entry: AttentionEntry) => void; attentionOpen: boolean; toggleAttention: () => void; historyRange: UsageRange; setHistoryRange: (range: UsageRange) => void }) {
  ...
  const historyTurns = view.workspaces.flatMap((workspace) => workspace.agents.flatMap((agent) => agent.usageTurns.filter((turn) => !turn.provisional && turn.observedAt).map((turn): NormalizedUsageTurn => ({ projectId: view.project.id, workspaceId: workspace.id, agentId: agent.id, turnId: turn.turnId ?? `${agent.id}-${turn.observedAt}`, observedAt: turn.observedAt!, startedAt: turn.startedAt ?? null, completedAt: turn.completedAt ?? null, model: turn.model ?? agent.model, canonicalModelId: turn.canonicalModelId, provider: turn.provider, displayName: turn.displayName, inputTokens: turn.inputTokens, cachedInputTokens: turn.cachedInputTokens, outputTokens: turn.outputTokens, contextUsedTokens: turn.contextUsedTokens, contextMaxTokens: turn.contextMaxTokens, costUsd: turn.costUsd, costState: turn.costState ?? (turn.costUsd === null ? "unknown" : "complete"), confidence: "high" }))));
  ...
  return (
    <>
      ...
      <View accessibilityLabel="Agent lifecycle summary" style={styles.summary}>
       ...
       <ModelUsagePanel models={history.models} workspaces={history.workspaces} range={historyRange} styles={styles} />
...
function HistoryCharts({ history, range, onRangeChange, styles, exportValue, onExport }: { history: HistoricalUsageProjection; range: UsageRange; onRangeChange: (range: UsageRange) => void; styles: PanelStyles; exportValue: string | null; onExport: () => void }) {
  ...
  return <View accessibilityLabel={`${USAGE_RANGE_LABELS[range]} usage charts; finalized turns only; provisional usage excluded from chart scales`} style={styles.historyCard}><View style={styles.headerRow}><View><Text accessibilityRole="header" style={styles.sectionTitle}>Usage history</Text><Text style={styles.label}>{historySourceLabel(history)} · {history.recordedTokens.toLocaleString()} recorded tokens</Text><Text style={styles.label}>Estimated costs use OpenCode Go pricing, source date {OPENCODE_GO_SOURCE_DATE}, version {OPENCODE_GO_PRICING_VERSION}.</Text></View><View style={styles.rangeTabs}>{(Object.keys(USAGE_RANGE_LABELS) as UsageRange[]).map((value) => <Pressable key={value} accessibilityRole="radio" accessibilityState={{ selected: range === value }} onPress={() => onRangeChange(value)} style={[styles.rangeTab, range === value ? styles.rangeTabActive : undefined]}><Text style={range === value ? styles.dismissLabel : styles.label}>{USAGE_RANGE_LABELS[value]}</Text></Pressable>)}</View></View><View style={styles.chartRow}><View style={styles.chartPanel}><Text style={styles.label}>Finalized tokens per {range === "24h" ? "hour" : "day"}</Text><View style={styles.chartPlot}>{buckets.map((bucket, index) => <View key={index} style={styles.chartBar}><View style={[styles.chartSegment, { height: `${bucket.output / maxBar * 100}%` }, styles.segmentOutput]} /><View style={[styles.chartSegment, { height: `${bucket.cached / maxBar * 100}%` }, styles.segmentCached]} /><View style={[styles.chartSegment, { height: `${bucket.fresh / maxBar * 100}%` }, styles.segmentFresh]} /></View>)}</View></View><View style={styles.chartPanel}><Text style={styles.label}>Cumulative fresh / cached / output</Text><View style={styles.linePlot}>{lineSegments("fresh", palette.fresh)}{lineSegments("cached", palette.cached)}{lineSegments("output", palette.output)}</View><View style={styles.chartAxis}><Text style={styles.label}>Start</Text><Text style={styles.label}>Now</Text></View></View></View><Pressable accessibilityRole="button" onPress={onExport} style={styles.touchTarget}><Text style={styles.label}>Prepare sanitized history for copying</Text></Pressable>{exportValue ? <Text selectable style={styles.label}>{exportValue}</Text> : null}</View>;
}

function ModelUsagePanel({ models, workspaces, range, styles }: { models: HistoricalUsageProjection["models"]; workspaces: HistoricalUsageProjection["workspaces"]; range: UsageRange; styles: PanelStyles }) {
  const maximum = Math.max(...models.map((bar) => bar.recordedTokens), 1);
  const workspaceMaximum = Math.max(...workspaces.map((bar) => bar.recordedTokens), 1);
  return <View accessibilityLabel={`Finalized model and workspace comparison for ${USAGE_RANGE_LABELS[range]}`} style={styles.usageSection}><Text accessibilityRole="header" style={styles.sectionTitle}>Usage comparison</Text><Text style={styles.label}>Model and provider totals; bars are scaled within this finalized date range.</Text><Text accessibilityRole="header" style={styles.sectionTitle}>Usage by model</Text><View style={styles.legend}><View style={styles.legendItem}><View style={styles.legendSwatchFresh} /><Text style={styles.label}>Fresh</Text></View><View style={styles.legendItem}><View style={styles.legendSwatchCached} /><Text style={styles.label}>Cached</Text></View><View style={styles.legendItem}><View style={styles.legendSwatchOutput} /><Text style={styles.label}>Output</Text></View></View>{models.length === 0 ? <Text style={styles.subtitle}>No finalized usage reported yet.</Text> : models.map((bar) => <ModelBar key={bar.key} bar={{ model: bar.displayName, provider: bar.provider, freshInputTokens: bar.freshInputTokens, cachedInputTokens: bar.cachedInputTokens, outputTokens: bar.outputTokens, totalTokens: bar.recordedTokens, reportedCostUsd: bar.reportedCostUsd, costState: bar.costState === "exact" || bar.costState === "free-model" ? "complete" : bar.costState === "estimated" ? "partial" : "unknown" }} maximum={maximum} styles={styles} />)}<Text accessibilityRole="header" style={styles.sectionTitle}>Usage by workspace</Text>{workspaces.length === 0 ? <Text style={styles.subtitle}>No finalized workspace usage reported yet.</Text> : workspaces.map((bar) => <View key={bar.key} accessibilityLabel={`${bar.displayName}, ${bar.recordedTokens} total tokens, ${bar.finalizedTurnCount} finalized turns, ${bar.costState} cost`} style={styles.barRow}><View style={styles.barHeader}><Text style={styles.modelName}>{bar.displayName}</Text><Text style={styles.tokenTotal}>{bar.recordedTokens.toLocaleString()} tokens</Text></View><View style={[styles.barTrack, { width: `${(bar.recordedTokens / workspaceMaximum) * 100}%` }]} /></View>)}</View>;
}
