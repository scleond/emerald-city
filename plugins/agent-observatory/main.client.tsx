...
import { historySourceLabel, projectHistoryForRange, sanitizedUsageExport, USAGE_RANGE_LABELS } from "./usage-history";
...
function ReadyContent({ view, styles, telemetry, lifecycle, setLifecycle, selectedAgentId, selectAgent, loadMore, timeline, attention, onDismiss, attentionOpen, toggleAttention, historyRange, setHistoryRange }: { view: ObservatoryViewModel; styles: PanelStyles; telemetry?: TelemetryDiagnostic; lifecycle?: AgentLifecycle; setLifecycle: (value: AgentLifecycle | undefined) => void; selectedAgentId: string | null; selectAgent: (id: string) => void; loadMore: (id: string) => void; timeline: Record<string, { entries: { label: string; summary: string; at: string }[]; error?: string; hasOlder: boolean }>; attention: AttentionEntry[]; onDismiss: (entry: AttentionEntry) => void; attentionOpen: boolean; toggleAttention: () => void; historyRange: UsageRange; setHistoryRange: (range: UsageRange) => void }) {
  ...
  const historyTurns = view.workspaces.flatMap((workspace) => workspace.agents.flatMap((agent) => agent.usageTurns.filter((turn) => !turn.provisional).map((turn): NormalizedUsageTurn => ({ projectId: view.project.id, workspaceId: workspace.id, agentId: agent.id, turnId: turn.turnId ?? `${agent.id}-${agent.updatedAt}`, observedAt: agent.updatedAt || new Date().toISOString(), startedAt: null, completedAt: agent.updatedAt || null, model: turn.model ?? agent.model, inputTokens: turn.inputTokens, cachedInputTokens: turn.cachedInputTokens, outputTokens: turn.outputTokens, contextUsedTokens: turn.contextUsedTokens, contextMaxTokens: turn.contextMaxTokens, costUsd: turn.costUsd, costState: turn.costUsd === null ? "unknown" : "complete", confidence: "high" }))));
